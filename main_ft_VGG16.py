...
trainlist = []
testlist = []
if mode == 3 or mode == 4: #mode 3 training by all subjects, mode 4 training with fine-tuning by all subjects
...
if mode == 0 or mode == 3: #mode 0 training by leave-one-person-out and mode 3 training by all subjects
	...
	model = load_model('./savemodel/Sakamoto_pretrained_VGG16/result.h5') #saveFilePath = str(sys.argv[2]) + "_" + color_mode + "_ini"
